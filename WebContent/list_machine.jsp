...
<body>
	<div class="container">
		...
		<div class="row" style="margin-top: 5px; margin-bottom: 10px;">
			<div class="col-sm-4">
				<p>Tìm kiếm thông tin các máy trong tiệm:</p>
				
			</div>
			<div class="col-sm-4">
				<form class="form-inline">
					<input class="form-control mr-sm-2" type="search"
						placeholder="Nhập tên máy" aria-label="Search">
		...
		<table class="table table-hover">
			...
			<tbody>
				<c:forEach items="${listComputer}" var="computer">
					<tr data-code="${computer.code}">
						<td>${computer.code}</td>
						<td>${computer.position}</td>
						<td><c:choose>
								<c:when test="${computer.status}">Đang hoạt động</c:when>
								<c:otherwise>Tắt</c:otherwise>
							</c:choose></td>
						<td><a
							href="${pageContext.request.contextPath}/machine?action=edit&code=${computer.code}"
